// export const PrimaryDark    = '#002984';
// export const PrimaryMain    = '#3f51b5';
// export const PrimaryLight   = '#757de8';

// export const SecondaryMain  ='#ff3d00';
// export const SecondaryDark  ='#c30000';
// export const SecondaryLight ='#ff7539';

export const PrimaryColorDark       = '#303F9F';
export const PrimaryColorDefault    = '#3F51B5';
export const PrimaryColorLight      = '#C5CAE9';

export const AccentColor            = '#FF5722';
export const AccentColorLight       = '#FFECB3';
export const TextPrimaryColor       = '#FFFFFF';

export const PrimaryTextColor       = '#212121';
export const SecondaryTextColor     = '#757575';
export const DividerColor           = '#BDBDBD';


